...
class Arc extends ecs_1.Component {
    ...
    getClosestPointTo(out, point) {
        const endAngle = this.getStartAngle(), startAngle = this.getEndAngle(), position = ecs_game_1.TransformComponent.getRequiredTransform(this.getRequiredEntity()).getPosition2(VEC2_0), distanceToCenter = gl_matrix_1.vec2.sub(VEC2_1, point, position), direction = gl_matrix_1.vec2.normalize(VEC2_2, distanceToCenter), angle = ecs_game_1.getAngleFromPoint(direction);
        if (angle < startAngle) {
