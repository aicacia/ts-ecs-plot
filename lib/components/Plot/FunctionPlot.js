...
class FunctionPlot extends PlotSection_1.PlotSection {
    ...
    getAsymptoteParts(points) {
        return this.fAsymptote
            ...
            for (let i = 0, j = 1, il = points.length; i < il && j < il; i++, j = i + 1) {
                ...
                if (b[0] > asymptote) {
                    current.push(gl_matrix_1.vec2.fromValues(asymptote, this.f(asymptote - ecs_game_1.EPSILON)));
                    const nextPoint = gl_matrix_1.vec2.fromValues(asymptote, this.f(asymptote + ecs_game_1.EPSILON));
                    current = [nextPoint];
                    out.push(current);
    ...
    getY(x) {
        return this.fAsymptote
            ...
            if (x <= asymptote - ecs_game_1.EPSILON || x >= asymptote + ecs_game_1.EPSILON) {
