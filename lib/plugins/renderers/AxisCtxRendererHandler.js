        ...
        this.getManager(Axis_1.AxisManager).map(function (manager) {
            return manager.getComponents().forEach(function (axis) {
                return renderer.render(function (ctx) {
                    var size = axis.getSize(), showTicks = axis.getShowTicks(), showNumbers = axis.getShowNumbers(), tickSize = axis.getTickSize(), halfTickSize = tickSize * 0.5, quaterTickSize = tickSize * 0.25;
                    ...
                    if (showTicks || showNumbers) {
                        for (var x = Math.floor(startX) - gridOffsetX, xl = Math.ceil(endX); x < xl; x += size) {
                            ...
                            if (showNumbers) {
                                ctx.scale(1, -1);
                                ctx.fillText((positionX + x).toString(), x + quaterTickSize, -quaterTickSize);
                                ctx.scale(1, -1);
                            }
                        }
                    }
                    ...
                    if (showTicks || showNumbers) {
                        for (var y = Math.floor(startY) - gridOffsetY, yl = Math.ceil(endY); y < yl; y += size) {
                            ...
                            if (showNumbers) {
                                ctx.save();
                                ctx.scale(1, -1);
                                ctx.fillText((positionY + y).toString(), quaterTickSize, -quaterTickSize);
                                ctx.restore();
                            }
                            ctx.restore();
                        }
