...
const ecs_2 = require("@aicacia/ecs");
...
class PlotSceneBuilder extends Builder_1.Builder {
    constructor() {
        super(new ecs_1.Scene()
            ...
            .addTag("camera")
            .addComponent(new ecs_2.Transform2D().setLocalScale(gl_matrix_1.vec2.fromValues(5, 5)), new ecs_2.Camera2DControl(), new ecs_2.Camera2D().setBackground(gl_matrix_1.vec4.fromValues(0.98, 0.98, 0.98, 1.0))))
            .addPlugin(new ecs_2.Time(), new ecs_2.Input())
            .maintain());
    }
    loop() {
        this.value.addPlugin(new ecs_2.Loop());
        return this;
    }
    eventLoop() {
        this.value.addPlugin(new ecs_2.EventLoop());
        return this;
    }
    disableCameraControl() {
        return this.camera((entity) => {
            entity.getRequiredComponent(ecs_2.Camera2DControl).setEnabled(false);
            return entity;
        });
    }
    entity(tags, updater) {
