...
class GridCtxRendererHandler extends web_1.CtxRendererHandler {
    onRender() {
        const camera = this.getCamera(), cameraTransform = ecs_1.TransformComponent.getRequiredTransform(camera.getRequiredEntity()), position = cameraTransform.getPosition2(VEC2_0), scale = this.getScale(), width = camera.getWidth() * scale, height = camera.getHeight() * scale, halfWidth = width * 0.5, halfHeight = height * 0.5, x = position[0], y = position[1], startX = -halfWidth, endX = halfWidth, startY = -halfHeight, endY = halfHeight, renderer = this.getRequiredRenderer(), matrix = gl_matrix_1.mat2d.fromTranslation(MAT2D_0, position);
        this.getManager(Grid_1.GridManager).map((manager) => manager.getComponents().forEach((grid) => renderer.render((ctx) => {
            ...
            ctx.strokeStyle = ecs_1.toRgba(grid.getColor());
            ctx.beginPath();
