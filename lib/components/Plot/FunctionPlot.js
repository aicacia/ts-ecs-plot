...
var FunctionPlot = /** @class */ (function (_super) {
    tslib_1.__extends(FunctionPlot, _super);
    ...
    FunctionPlot.prototype.getAsymptoteParts = function (points) {
        ...
        return this.fAsymptote
            ...
            for (var i = 0, j = 1, il = points.length; i < il && j < il; i++, j = i + 1) {
                ...
                if (b[0] > asymptote) {
                    current.push(gl_matrix_1.vec2.fromValues(asymptote, _this.f(asymptote - ecs_game_1.EPSILON)));
                    var nextPoint = gl_matrix_1.vec2.fromValues(asymptote, _this.f(asymptote + ecs_game_1.EPSILON));
                    current = [nextPoint];
                    out.push(current);
