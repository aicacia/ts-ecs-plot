...
exports.ShapeBuilder = exports.ShapePoint = void 0;
...
class ShapeBuilder extends Builder_1.Builder {
    constructor() {
        super(new ecs_1.Entity().addTag("shape").addComponent(new ecs_game_1.Transform2D()));
        this.points = [];
    }
    addPoint(...points) {
    ...
    build() {
        const points = this.points.reduce((points, point, index) => {
            points.push(new ecs_1.Entity()
                ...
                .addComponent(new components_1.Point().set(point.getPointData()), new ecs_game_1.Transform2D().setLocalPosition2(point.getPosition())));
            return points;
        }, []);
        const lines = points.reduce((lines, point, index) => {
            ...
            lines.push(new ecs_1.Entity()
                ...
                .addComponent(new components_1.Line().setStart(prevPoint).setEnd(point)));
            return lines;
        }, []);
        points.forEach((point, index) => {
            ...
            point.addChild(new ecs_1.Entity()
                ...
                .addComponent(new ecs_game_1.Transform2D(), new components_1.Arc()
                .setStart(prevPoint)
                .setEnd(nextPoint)
...
function getDistanceBetween(a, b) {
    return gl_matrix_1.vec2.len(gl_matrix_1.vec2.sub(GET_DISTANCE_BETWEEN_VEC2_2, ecs_game_1.TransformComponent.getRequiredTransform(a).getLocalPosition2(GET_DISTANCE_BETWEEN_VEC2_0), ecs_game_1.TransformComponent.getRequiredTransform(b).getLocalPosition2(GET_DISTANCE_BETWEEN_VEC2_1)));
}
